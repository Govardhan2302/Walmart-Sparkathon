 ...
 // JDBC connection details
    ...
    try {
        ...
        if (conn != null) {
            ...
            String sql = "SELECT branch, SUM(quantity) AS quantity FROM sales GROUP BY branch HAVING SUM(quantity) > AVG(quantity) ORDER BY quantity DESC LIMIT 1;";
            stmt = conn.createStatement();
            rs = stmt.executeQuery(sql);

            while (rs.next()) {
            	city = rs.getString("branch");
                total_revenue = rs.getInt("quantity");
                %>
