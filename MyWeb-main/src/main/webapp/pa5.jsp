...
<!-- Navigation Bar -->
    ...
    <h2>Highest COGS by Month</h2>
<table class="table table-bordered">
        <thead>
            <tr>
                <th width=50>Month Name</th>
                <th width=50>Total COGS</th>
            </tr>
        </thead>
    <%
 // JDBC connection details
    ...
    try {
        ...
        if (conn != null) {
            ...
            String sql = "SELECT month_name, SUM(cogs) AS total_cogs FROM sales GROUP BY month_name ORDER BY total_cogs DESC";
            stmt = conn.createStatement();
            rs = stmt.executeQuery(sql);

            while (rs.next()) {
            	monthName = rs.getString("month_name");
                total_cogs = rs.getInt("total_cogs");
                %>
